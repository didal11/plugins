...
## 7) 의뢰 타입 확장 관점에서의 현재 구조 해석

- 현재 길드 발행 타입은 코드 상 `explore`/`gather`/`craft`/`trade` 4종입니다.
- 현재 자동 발행 로직은 `gather`를 발행할 때 실제 액션명은 리소스 키 접두사(`herb/tree/ore/...`)를 보고
  `약초채집`/`벌목`/`채광` 같은 채집 액션으로 분기합니다.
- 즉 **엔진 레벨에서는 이미 “리소스 엔티티 기반 채집 의뢰”를 별도 액션으로 운용**하고 있고,
  표시 문자열은 `gather/craft/trade`에 대해 각각 `OO 채집/제작/교역`으로 표기됩니다.
- 장기적으로 `탐색/채집/제작/교역` 4분류로 확장하려면,
  `GuildIssueType`에 타입을 추가하고(예: `GATHER`, `CRAFT`, `TRADE`) 
  `_jobs_for_issue()`/발행 로직/표시 문자열(`display_action_name`)을 타입 기준으로 분리하면
  현 구조와 잘 맞습니다.
