...
def _load_people(path: Path, fallback: List[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for it in raw if isinstance(raw, list) else []:
        if not isinstance(it, dict):
            continue
        normalized = _normalize_person(it, races)
        if normalized:
            out.append(normalized)
...
def save_npc_templates(npcs: List[Dict[str, object]]) -> None:
    ...
    clean = []
    for n in npcs:
        normalized = _normalize_person(n, races)
        if normalized:
            clean.append(normalized)
...
def save_monster_templates(monsters: List[Dict[str, object]]) -> None:
    ...
    clean = []
    for m in monsters:
        normalized = _normalize_person(m, races)
        if normalized:
            clean.append(normalized)
...
def load_entities() -> List[Dict[str, object]]:
    ...
    for row in raw:
        if not isinstance(row, dict):
            continue
        normalized = _normalize_entity(row)
        if normalized:
            out.append(normalized)
