...
class EditorApp(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        ttk.Label(top, text="JSON 파일은 상단 탭 버튼으로 선택합니다.").pack(side="left")

        self.current_tab_var = tk.StringVar(value="")
        ttk.Label(top, textvariable=self.current_tab_var).pack(side="right")

        self.tab_button_bar = ttk.Frame(self)
        self.tab_button_bar.pack(fill="x", padx=8, pady=(6, 0))
    ...
    def _build_tabs(self) -> None:
        ...
        for filename, (tab_title, schema) in TAB_SCHEMAS.items():
            ...
            self._add_tab_button(tab_title)
    ...
    def _add_tab_button(self, tab_title: str) -> None:
        btn = ttk.Button(self.tab_button_bar, text=tab_title, command=lambda t=tab_title: self._select_tab(t))
        btn.pack(side="left", padx=3, pady=2)
        self.tab_buttons[tab_title] = btn

    def _select_tab(self, tab_title: str) -> None:
        tab_count = self.notebook.index("end")
        for index in range(tab_count):
            if self.notebook.tab(index, "text") == tab_title:
    ...
    def _on_tab_changed(self, _event=None) -> None:
        ...
        self.current_tab_var.set(f"현재 탭: {tab_title}")
