...
VALID_GENDERS = getattr(_edata, "VALID_GENDERS", ["남", "여", "기타"])
VALID_JOBS = getattr(_edata, "VALID_JOBS", ["모험가", "농부", "어부", "대장장이", "약사"])

load_item_defs = _edata.load_item_defs
load_job_defs = _edata.load_job_defs
load_sim_settings = _edata.load_sim_settings
save_item_defs = _edata.save_item_defs
save_job_defs = _edata.save_job_defs
save_sim_settings = _edata.save_sim_settings

load_npc_templates = getattr(_edata, "load_npc_templates", lambda: [])
save_npc_templates = getattr(_edata, "save_npc_templates", lambda _rows: None)
load_monster_templates = getattr(_edata, "load_monster_templates", lambda: [])
save_monster_templates = getattr(_edata, "save_monster_templates", lambda _rows: None)
load_races = getattr(_edata, "load_races", lambda: [{"name": "인간", "is_hostile": False, "str_bonus": 0, "agi_bonus": 0, "hp_bonus": 0, "speed_bonus": 0.0}])
save_races = getattr(_edata, "save_races", lambda _rows: None)
load_entities = getattr(_edata, "load_entities", lambda: [])
save_entities = getattr(_edata, "save_entities", lambda _rows: None)
...
class EditorApp(tk.Tk):
    ...
    def _refresh_people_race_options(self) -> None:
        values = self._race_names()
        for mode in ("npc", "monster"):
            entries = getattr(self, f"{mode}_entries", None)
            if not entries:
                continue
            race_cb = entries.get("race")
            if race_cb is not None:
                race_cb["values"] = values
    ...
    def _build_race_tab(self):
        ...
        def add_row():
            ...
            self._refresh_people_race_options()
    ...
    def _save_races(self):
        self._save_list(self.races, save_races, "종족")
        self._refresh_people_race_options()
